...
BEGIN
    IF INSERTING THEN
        IF :NEW.jeton = 0 THEN
            INSERT INTO auteur@FST1 VALUES(:NEW.id_aut, :NEW.nom_auteur,1);
        END IF;
    ELSIF UPDATING THEN
        IF :NEW.jeton = 0 THEN
            UPDATE auteur@FST1 SET nom_auteur= :NEW.nom_auteur, jeton=1 WHERE id_aut=:NEW.id_aut;
        END IF;
    ELSIF DELETING THEN
        IF :OLD.jeton = 0 THEN
            DELETE FROM auteur@FST1 WHERE id_aut=:OLD.id_aut;
        END IF;
    END IF;
EXCEPTION
    WHEN DUP_VAL_ON_INDEX THEN
