...
BEGIN
    SELECT COUNT(*) INTO v_count FROM pret_2 WHERE id_etu=p_id_etu AND id_ouv=p_id_ouv AND date_retour IS NULL;
    ...
    ELSIF v_nb_emprunt >= 3 THEN
        res := -2;
    ELSE
        SELECT stock INTO v_stock FROM OUVRAGE_2 WHERE id_ouv=p_id_ouv;
        IF v_stock <= 0 THEN
            res := -3;
        ELSE
            INSERT INTO PRET_2 (id_pret, date_empreunt, date_retour, date_retour_limite, id_etu, id_ouv, id_pers)
            VALUES (seq_pret.nextval, p_date_emprunt, null, p_date_retour_limite, p_id_etu, p_id_ouv, p_id_emp);
            UPDATE OUVRAGE_2 SET stock = stock-1 WHERE id_ouv=p_id_ouv;
...
BEGIN
    OPEN pret_cursor FOR
        ...
        FROM pret_2 p, ouvrage_2 o WHERE p.id_ouv=o.id_ouv AND id_etu=p_id_etu;
    RETURN pret_cursor;
END;
/
...
CREATE OR REPLACE FUNCTION rendreOuvrage(p_id_pret NUMBER)
    ...
    v_id_ouv NUMBER:=0;
BEGIN
    SELECT date_retour, id_etu, id_ouv INTO v_date_retour, v_id_etu, v_id_ouv FROM pret_2 WHERE id_pret=p_id_pret;
    IF v_date_retour IS NOT NULL THEN
        res:=-1;
    ELSE
        UPDATE pret_2 SET date_retour= SYSDATE WHERE id_pret=p_id_pret;
        UPDATE etudiant_emprunt SET nb_emprunts = nb_emprunts-1,jeton=0 WHERE id_etu=v_id_etu;
        UPDATE ouvrage_2 SET stock = stock+1 WHERE id_ouv=v_id_ouv;
        commit;
        res:=1;
    END IF;
    RETURN res;
